...
export const getEmailStats = async (req: Request, res: Response) => {
  try {
    ...
    const stats = await logService.getEmailStats(orgId, apiKeyUsed as string | undefined);
    res.json(stats);
  } catch (error) {
...
export const getEmailAnalytics = async (req: Request, res: Response) => {
  try {
    const orgId = req.params.orgId as string;
    const { apiKeyUsed } = req.query;
    const analytics = await logService.getEmailAnalytics(orgId, apiKeyUsed as string | undefined);
    res.json(analytics);
  } catch (error) {
