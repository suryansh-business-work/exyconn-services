...
export const createEmailTemplate = async (req: Request, res: Response) => {
  try {
    ...
    const template = await templateService.createEmailTemplate(orgId, validatedData);
    res.status(201).json(template);
  } catch (error) {
    if (error instanceof Error && error.message.includes('already exists')) {
      res.status(409).json({ error: error.message });
    } else if (error instanceof Error && error.message.includes('MJML compilation')) {
...
export const updateEmailTemplate = async (req: Request, res: Response) => {
  try {
    ...
    const template = await templateService.updateEmailTemplate(orgId, templateId, validatedData);
    res.json(template);
  } catch (error) {
    if (error instanceof Error && error.message.includes('not found')) {
      res.status(404).json({ error: error.message });
