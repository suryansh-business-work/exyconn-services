...
export const historyService = {
  list: async (orgId: string, params: ListParams) => {
    ...
    const [data, total] = await Promise.all([
      SiteCheckResult.find(query).sort({ timestamp: -1 }).skip(skip).limit(limit).lean(),
  ...
  get: async (orgId: string, checkId: string): Promise<ISiteCheckResult | null> => {
  ...
  getStats: async (orgId: string) => {
    ...
    const [totalMonitors, activeMonitors, statusCounts, avgResponseTime, last24hChecks] =
      await Promise.all([
        SiteMonitor.countDocuments({ organizationId: orgObjectId }),
        SiteMonitor.countDocuments({ organizationId: orgObjectId, isActive: true }),
    ...
    return {
      ...
      uptimePercentage:
        totalChecks24h > 0 ? Math.round((last24hChecks / totalChecks24h) * 100) : 100,
    };
  },
};
