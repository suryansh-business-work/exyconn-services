...
export const imageKitConfigApi = {
  list: async (orgId: string, params: ListParams = {}): Promise<ImageKitConfigListResponse> => {
    const response = await axios.get(`${API_BASE}/organizations/${orgId}/imagekit/config`, {
      params,
    });
  ...
  getDefault: async (orgId: string): Promise<ImageKitConfig> => {
    const response = await axios.get(`${API_BASE}/organizations/${orgId}/imagekit/config/default`);
    return response.data;
  },

  create: async (orgId: string, data: ImageKitConfigFormValues): Promise<ImageKitConfig> => {
    const response = await axios.post(`${API_BASE}/organizations/${orgId}/imagekit/config`, data);
    return response.data;
  },
  ...
  delete: async (orgId: string, configId: string): Promise<void> => {
    await axios.delete(`${API_BASE}/organizations/${orgId}/imagekit/config/${configId}`);
  },
...
export const fileUploadApi = {
  uploadSingle: async (
    ...
    configId: string,
    file: File,
    options?: { folder?: string; tags?: string[]; useUniqueFileName?: boolean; apiKey?: string }
  ...
  delete: async (orgId: string, fileId: string): Promise<void> => {
    await axios.delete(`${API_BASE}/organizations/${orgId}/imagekit/files/${fileId}`);
  },
};
...
export const uploadHistoryApi = {
  list: async (
    ...
    const response = await axios.get(`${API_BASE}/organizations/${orgId}/imagekit/history`, {
      params,
    });
    return response.data;
  },
  ...
  getStats: async (orgId: string): Promise<UploadStats> => {
    const response = await axios.get(`${API_BASE}/organizations/${orgId}/imagekit/stats`);
    return response.data;
  },
};
