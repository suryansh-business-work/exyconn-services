...
export const organizationApi = {
  ...
  create: async (data: Omit<OrganizationFormValues, 'orgApiKeys'>): Promise<Organization> => {
    const response = await axios.post(`${API_BASE}/organizations`, data);
    return response.data.data;
  },

  update: async (id: string, data: Partial<OrganizationFormValues>): Promise<Organization> => {
  ...
  addApiKey: async (
    ...
    const response = await axios.post(`${API_BASE}/organizations/${orgId}/api-keys`, { keyName });
    return response.data.data;
  },

  removeApiKey: async (orgId: string, apiKey: string): Promise<void> => {
    await axios.delete(`${API_BASE}/organizations/${orgId}/api-keys/${encodeURIComponent(apiKey)}`);
  },
};
