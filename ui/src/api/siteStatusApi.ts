...
export const siteMonitorApi = {
  list: async (
    ...
    const res = await fetch(`${API_BASE}/organizations/${orgId}/site-status/monitors?${query}`, {
  ...
  get: async (orgId: string, monitorId: string, apiKey?: string): Promise<SiteMonitorConfig> => {
  ...
  create: async (
    ...
    const res = await fetch(`${API_BASE}/organizations/${orgId}/site-status/monitors`, {
      method: 'POST',
      headers: getHeaders(apiKey),
  ...
  delete: async (orgId: string, monitorId: string, apiKey?: string): Promise<void> => {
    const res = await fetch(
      `${API_BASE}/organizations/${orgId}/site-status/monitors/${monitorId}`,
      {
        method: 'DELETE',
  ...
  checkNow: async (orgId: string, monitorId: string, apiKey?: string): Promise<SiteCheckResult> => {
    const res = await fetch(
...
export const siteCheckHistoryApi = {
  list: async (
    ...
    const res = await fetch(`${API_BASE}/organizations/${orgId}/site-status/history?${query}`, {
      headers: getHeaders(apiKey),
    });
  ...
  get: async (orgId: string, checkId: string, apiKey?: string): Promise<SiteCheckResult> => {
    const res = await fetch(`${API_BASE}/organizations/${orgId}/site-status/history/${checkId}`, {
      headers: getHeaders(apiKey),
    });
  ...
  getStats: async (orgId: string, apiKey?: string): Promise<SiteMonitorStats> => {
    const res = await fetch(`${API_BASE}/organizations/${orgId}/site-status/stats`, {
      headers: getHeaders(apiKey),
    });
