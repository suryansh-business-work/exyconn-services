...
export const APP_DATA: AppDefinition[] = [
  ...
  },
  {
    id: 'feature-flags',
    name: 'Feature Flags',
    description: 'Control feature rollouts with toggles, percentage rollouts, and targeting rules.',
    iconName: 'Flag',
    ...
    children: [
      {
        ...
        keywords: ['image', 'cdn', 'optimization', 'transform', 'upload', 'file'],
      },
    ],
  },
  ...
  // Monitoring & Analytics
  {
    ...
    keywords: ['uptime', 'health', 'monitoring', 'ping', 'ssl', 'dns', 'screenshot'],
  },
  ...
  },
  {
    ...
    keywords: ['scheduler', 'task', 'recurring', 'automation', 'job', 'webhook'],
  },
  {
    id: 'global-search',
    name: 'Global Search',
  ...
  },
  {
    ...
    keywords: ['analytics', 'tracking', 'click', 'heatmap', 'user behavior', 'meta'],
  },
...
export function searchApps(query: string): AppDefinition[] {
  ...
  return getAllAppsFlat().filter((app) => {
    ...
    const keywordMatch = app.keywords?.some((k) => k.toLowerCase().includes(lowerQuery));
 ...
 */
export function getStatusColor(status: AppStatus): 'success' | 'warning' | 'default' {
  switch (status) {
    case 'live':
      return 'success';
    case 'dev':
