...
const AIDashboard = () => {
  ...
  const statCards = [
    ...
    { label: 'Total Chats', value: chatStats?.totalChats ?? 0, icon: <Chat />, color: '#ed6c02' },
    {
  ...
  return (
    <Box>
      ...
      {companyStats?.byProvider && Object.keys(companyStats.byProvider).length > 0 && (
        <Paper variant="outlined" sx={{ p: 2, mt: 3 }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 1 }}>
            Companies by Provider
          </Typography>
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            {Object.entries(companyStats.byProvider).map(([provider, count]) => (
      ...
      <Paper variant="outlined" sx={{ p: 3, mt: 3 }}>
        ...
        <Typography variant="body2" color="text.secondary">
          Integrate multiple AI providers (OpenAI, Gemini, Anthropic) and manage chat conversations
          with history tracking. Set token limits and history size in chat settings.
        </Typography>
      </Paper>
    </Box>
  );
