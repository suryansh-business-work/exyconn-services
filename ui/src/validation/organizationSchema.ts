...
export const organizationValidationSchema = Yup.object({
  ...
  orgSlug: Yup.string()
    ...
    .matches(/^[a-z][a-zA-Z0-9]*$/, 'Slug must be camelCase starting with lowercase')
  ...
  orgApiKeys: Yup.array()
    ...
    .test('unique-key-names', 'Duplicate API key names are not allowed', function (value) {
      if (!value || value.length === 0) return true;
      const keyNames = value.map((k) => k.keyName?.toLowerCase()).filter(Boolean);
